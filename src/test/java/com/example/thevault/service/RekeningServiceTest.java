...
class RekeningServiceTest {
 ...
 /*   @Test
    void wijzigSaldo() {
        ...
        Mockito.when(mockRepo.wijzigSaldoVanKlant(bestaandeKlantVoorWijzigSaldo, new BigDecimal("2000"))).thenReturn(bestaandeKlantVoorWijzigSaldo.getRekening());

        Rekening rekeningActual = rekeningServiceTest.wijzigSaldo(bestaandeKlantVoorWijzigSaldo.getRekening(), new BigDecimal("2000"));
        System.out.println(rekeningActual);
        BigDecimal expected = new BigDecimal("2000");
        BigDecimal actual = rekeningActual.getBigDecimalSaldo();
